...
*The dtype of the `actualShipDate` series is `datetime64[ns]`, which can be verified using:
...
:::
The dtype of `actualShipDate` is `datetime64[ns]`.  

This is important because it ensures that `actualShipDate` can be **compared directly** with `plannedShipDate` and used in **date arithmetic**, like calculating how many days a shipment was late.  

If these columns were strings instead of datetime objects, comparisons or calculations could give **wrong or unintuitive results**, so having the correct dtype is essential for accurate analysis.
...
date_str1 = "04-11-2025"
date_str2 = "05-20-2024"
print("String comparison:", date_str1 > date_str2)

# Convert strings to datetime objects and compare
import pandas as pd
date1 = pd.to_datetime(date_str1)
date2 = pd.to_datetime(date_str2)
print("Datetime comparison:", date1 > date2)
```

::: {.callout-tip}
## Your Answer: Debug This Code
The lateStatement assignment fails because it tries to reference shipments_df['is_late'] before the .assign() operation has finished creating that column, leading to a KeyError. Additionally, a standard Python if/else statement is not "vectorized," meaning it cannot process an entire column of data at once.

To fix this, we use a lambda function to reference the "live" state of the DataFrame and np.where() to apply the conditional logic across all rows efficiently.
```{python}
import numpy as np
...
shipments_with_lateness = (
    ...
    .assign(
        is_late=lambda df: df['actualShipDate'] > df['plannedShipDate'],
        days_late=lambda df: (df['actualShipDate'] - df['plannedShipDate']).dt.days,
        # Use lambda 'df' to access the new column and np.where for the logic
...
# Preview the result
print(shipments_with_lateness[['shipID', 'is_late', 'lateStatement']].head())
```
:::
...
*Both `.query('is_late == True')` and boolean indexing like 
`shipments_with_lateness[shipments_with_lateness['is_late'] == True]`
filter rows based on a condition.

The difference is mainly in syntax and readability:

- Boolean indexing directly references the DataFrame and column.
- `.query()` uses a string expression that feels similar to SQL.

For simple conditions, `.query()` is often more readable. However, boolean indexing can be clearer when using complex Python logic.*

:::

::: {.callout-tip}
## Your Answer: Additional Row Querying
*We can use a variable like `late_threshold` inside `.query()` by using the `@` symbol.

For example, if we want shipments that are at least 5 days late:
...
*`.drop(columns=['quantity'])` removes specific columns from the DataFrame, while `.filter()` selects only the columns you explicitly want to keep.

The difference is strategic:

- **`.drop()`** is useful when you want to remove just a few unnecessary columns and keep everything else.
- **`.filter()`** is better when you only want a small, specific set of columns and want to explicitly control what remains.

You would typically use `.drop()` when cleaning minor unwanted data, and `.filter()` when narrowing the dataset to only the variables needed for a specific analysis.*
:::

::: {.callout-tip}
## Your Answer: Handling Missing Data
*If you use `.dropna()` without specifying `subset`, pandas will remove any row that contains a missing value in *any column. This can result in losing more data than intended.

In contrast, `.dropna(subset=['plannedShipDate', 'actualShipDate'])` only removes rows where those specific columns contain missing values.

Being selective is important because some columns (like optional metadata or text notes) may contain missing values that do not affect the analysis. Dropping rows based on all columns could unnecessarily reduce the dataset and remove valid observations.*
:::
...
*`ascending=False` sorts values from highest to lowest, while `ascending=True` sorts values from lowest to highest.

To sort by multiple columns, you can pass a list of columns to `.sort_values()`.  
For example, to sort first by whether a shipment is late and then by how many days late:

```{python}
# Question 1: Sorting Strategies
# We must start with the DataFrame name 'clean_shipments' 
# and remove the dots at the start and end.
sorted_multi_example = (
    clean_shipments
    .sort_values(['is_late', 'days_late'], ascending=[False, False])
)

# Preview the result
print(sorted_multi_example[['shipID', 'is_late', 'days_late']].head())
```
...
When you sort a DataFrame, pandas rearranges the rows but keeps the original index values. This can result in an index that is out of order or misleading.

Using `.reset_index(drop=True)` creates a new, sequential index that matches the sorted order of the data.

If the index is not reset, it can cause confusion during analysis or when referencing rows by index, especially in reporting or visualization. 
:::
...
*In pandas, boolean values are treated as numeric when performing aggregation. Internally, `True` is equivalent to `1` and `False` is equivalent to `0`.

Because of this:
- `sum()` on a boolean column counts how many values are `True`.
- `mean()` calculates the proportion of `True` values (since it averages 1s and 0s).

In this case, `sum()` on `is_late` counts the number of late shipments, and `mean()` calculates the percentage of shipments that were late.*
...
*A left join (`how='left'`) keeps all rows from the left DataFrame (in this case, `clean_shipments`) and only brings in matching data from `product_line_df`. 

Professors often recommend left joins because they prevent accidental data loss. If we used an inner join instead, any shipment with a `partID` that does not exist in `product_line_df` would be dropped entirely.

To check whether any shipments were lost during the merge, we can compare the number of rows before and after merging:

- `len(clean_shipments)`
- `len(shipments_with_category)`

If the counts are different when using a left join, something is wrong. We can also check for missing categories using:
`shipments_with_category['prodCategory'].isna().sum()`

This tells us how many shipments did not find a matching product category.*
:::

::: {.callout-tip}
## Your Answer: Key Column Matching
*If there are duplicate `partID` values in `product_line_df`, the merge will create duplicated rows in the result. 

This happens because pandas performs a many-to-one or many-to-many join. If a single shipment matches multiple rows in `product_line_df`, it will be repeated once for each match.

This can artificially inflate row counts and distort aggregations, leading to incorrect results. For this reason, it is important to ensure that the key column (`partID`) is unique in the lookup table before merging.*
:::
...
*`.groupby('prodCategory')` splits the DataFrame into separate groups, one for each unique value in the `prodCategory` column. Internally, pandas creates a mapping from each category to the rows belonging to that category.

We use `.agg()` after grouping to apply summary functions (like `sum()`, `mean()`, or `max()`) to each group. Without `.agg()`, the grouped object is not summarized; it is a `DataFrameGroupBy` object that cannot be directly printed as a meaningful table of metrics.*

:::

::: {.callout-tip}
## Your Answer: Multi-Level Grouping
*Grouping by `['shipID', 'prodCategory']` creates a hierarchical split: first by shipment, then by category within each shipment. 

This answers questions like: “Within each shipment, how are late shipments distributed across categories?” versus grouping by `prodCategory` alone, which only gives aggregate service metrics per category across all shipments. 

Multi-level grouping is useful when we want to analyze combinations of identifiers or understand patterns at a more granular level.*
...
*This comprehensive analysis answers the business question: “How often do shipments include products from more than one category, and which shipments have late items?”  

Grouping by `['shipID', 'prodCategory']` allows us to see lateness and other metrics at the shipment-category level. This is different from grouping by `'prodCategory'` alone, which only gives category-level metrics across all shipments, losing shipment-specific detail.  

From this analysis, ZappTech’s management can gain insights such as:  
- The proportion of shipments that span multiple categories, which may indicate cross-category ordering behavior.  
- Identifying shipments that are more prone to delays when multiple categories are involved, helping in resource planning or prioritizing logistics improvements.  
- Understanding patterns that can inform decisions about bundling, shipping optimization, or category-specific interventions to improve service levels.*
...
### 📊 Service Level by Product Category Visualization

This visualization shows the on-time delivery percentage for each product category.  
It helps identify which product categories experience more shipment delays.

```{python}
#| echo: false
#| warning: false

# Calculate service level (on-time %)
service_plot_data = (
    shipments_with_category
    .groupby('prodCategory')
    .agg(on_time_rate=('is_late', lambda x: (1 - x.mean()) * 100))
    .reset_index()
)

# Create visualization
plt.figure(figsize=(10,6))
sns.barplot(data=service_plot_data, x='prodCategory', y='on_time_rate')
